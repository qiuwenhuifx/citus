							  ...
							  AlterTableStmt *stmt, AlterTableType subtype)
{
	...
	else if (constraint->contype == CONSTR_CHECK)
	{
		...
		{
			/* preprocess should've already thrown a better error for that */
			ereport(ERROR, (errmsg("unexpectedly got a check constraint as "
								   "part of an add column subcommand")));
		}
